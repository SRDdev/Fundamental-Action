...
#         x = self.dropout(F.gelu(self.ln2(self.fc2(x))))
#         x = F.gelu(self.ln3(self.fc3(x)))
#         x = self.dropout(F.gelu(self.ln4(self.fc4(x))))
#         x = F.gelu(self.ln5(self.fc5(x)))
#         x = self.dropout(F.gelu(self.ln6(self.fc6(x))))
#         x = F.gelu(self.ln7(self.fc7(x)))
#         x = F.gelu(self.ln8(self.fc8(x)))
#         x = self.fc9(x)
#         return x
...
This file contains the implementation of a Neural Network with Layer Normalization that takes in configurable 
input size, hidden sizes, and outputs an 80 Point 4D Trajectory.
"""
import torch
import torch.nn as nn
...
class TrajectoryNet(nn.Module):
    """
    A Neural Network with Layer Normalization that takes in configurable input size, hidden sizes,
    and outputs an 80 Point 4D Trajectory.
    """
    def __init__(self, config):
        """
        ...
        self.fc5 = nn.Linear(hidden_size * 2, output_size)  # Output layer (80 points * 4 dimensions)
